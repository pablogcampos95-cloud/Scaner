...
import {
  createArea,
  createPerfilOperativo,
  deactivateArea,
  deactivatePerfilOperativo,
  getAreasActivas,
  getPerfilesOperativosActivos,
  updateArea,
  updatePerfilOperativo,
...
export default function AdminCatalogs() {
  ...
  const savePerfil = async (event) => {
    ...
    try {
      ...
      closeModal();
      load();
      setState((prev) => ({ ...prev, success: 'Perfil guardado correctamente.', error: '' }));
    } catch (error) {
      setState((prev) => ({ ...prev, error: error.message, success: '' }));
    }
  };
    ...
    { key: 'estado', header: 'Estado', render: (row) => <span className={`badge status--${row.estado === 'activa' ? 'completada' : 'vencida'}`}>{row.estado}</span> },
    {
      ...
      render: (row) => (
        <div className="table-actions">
          <button type="button" onClick={() => { setAreaForm(row); setActiveModal('area'); }}>Editar</button>
          <button type="button" onClick={() => deactivateArea(row.id).then(load)}>{row.estado === 'activa' ? 'Desactivar' : 'Activar'}</button>
        </div>
      ),
    },
  ];
  ...
  const perfilColumns = [
    ...
    { key: 'estado', header: 'Estado', render: (row) => <span className={`badge status--${row.estado === 'activo' ? 'completada' : 'vencida'}`}>{row.estado}</span> },
    {
      ...
      render: (row) => (
        <div className="table-actions">
          <button type="button" onClick={() => { setPerfilForm(row); setActiveModal('perfil'); }}>Editar</button>
          <button type="button" onClick={() => deactivatePerfilOperativo(row.id).then(load)}>{row.estado === 'activo' ? 'Desactivar' : 'Activar'}</button>
        </div>
      ),
    },
  ];
    ...
    <section className="page-stack">
      <div className="dashboard-hero">
        <div>
          ...
          <h1>Áreas y perfiles</h1>
          <p>Administra áreas y perfiles operativos para segmentar evaluaciones y postulantes.</p>
        </div>
        <div className="toolbar-actions">
          <button className="primary-button compact" type="button" onClick={() => { setAreaForm(emptyArea); setActiveModal('area'); }}>Nueva área</button>
          <button className="secondary-button compact" type="button" onClick={() => { setPerfilForm(emptyPerfil); setActiveModal('perfil'); }}>Nuevo perfil</button>
        </div>
      </div>
      ...
      {activeModal === 'area' ? (
        <div className="modal-backdrop">
          <form className="modal-card" onSubmit={saveArea}>
            <div className="modal-header">
              <div>
                <h2>{areaForm.id ? 'Editar área' : 'Nueva área'}</h2>
                <p>Define un segmento operativo para asociar evaluaciones y postulantes.</p>
              </div>
      ...
      {activeModal === 'perfil' ? (
        <div className="modal-backdrop">
          <form className="modal-card" onSubmit={savePerfil}>
            <div className="modal-header">
              <div>
                <h2>{perfilForm.id ? 'Editar perfil' : 'Nuevo perfil'}</h2>
                <p>Define el perfil operativo evaluable dentro de la plataforma.</p>
              </div>
              <button className="modal-close" type="button" onClick={closeModal}>×</button>
            </div>
